...
static inline void
wait_spin(void) {
#if defined(_MSC_VER) // 如果是 MSC 编译器
// ##
#if defined(_M_ARM64)
	__yield();
#else
...
// ##
	...
	__asm__ volatile("pause" ::: "memory"); // 自旋 
...
#endif
#else
#define TLS_MODEL __attribute__((tls_model("initial-exec"))) // 一般实现，非MSC 安卓
// #define TLS_MODEL
#endif
...
set_thread_heap(heap_t* heap) {
	global_thread_heap = heap; // 过河拆桥，然后将其设置为自己的heap
	if (heap && (heap->id != 0)) { // 如果成功获得heap了
		rpmalloc_assert(heap->id != 0, "Default heap being used");
		heap->owner_thread = get_thread_id(); // heap被此线程id持有
	}
#if PLATFORM_WINDOWS // WINDOWS
	FlsSetValue(fls_key, heap);
#else	// 一般实现
	pthread_setspecific(pthread_key, heap); // 绑定到线程自己
#endif
}

static heap_t*
get_thread_heap_allocate(void) {
	heap_t* heap = heap_allocate(0); // 获取heap
...
get_size_class(size_t size) {
	uintptr_t minblock_count = (size + (SMALL_GRANULARITY - 1)) / SMALL_GRANULARITY; // 向上对齐16
...
get_page_aligned_size(size_t size) {
	size_t unalign = size % global_config.page_size; // 对齐一页
	if (unalign) // 如果有，则加上一页的大小减去没对齐的部分
...
#if ENABLE_DECOMMIT
	if (global_config.disable_decommit) { // 如果不允许decommit 直接返回
		return 0;
	}
#if PLATFORM_WINDOWS // 忽略
...
#if RPMALLOC_FIRST_CLASS_HEAPS // 0
	return (!page->heap->owner_thread || (page->heap->owner_thread == get_thread_id()));
#else // global_thread_id == page->heap->thread 返回1
	return (page->heap->owner_thread == get_thread_id());
#endif
}

static inline block_t*
page_block_start(page_t* page) { // 返回 page + 128
	return pointer_offset(page, PAGE_HEADER_SIZE);
}

static inline block_t*
page_block(page_t* page, uint32_t block_index) { // 开头 + size * index
	return pointer_offset(page, PAGE_HEADER_SIZE + (page->block_size * block_index));
}

static inline uint32_t
page_block_index(page_t* page, block_t* block) {
	block_t* block_first = page_block_start(page); // 获取page 开始存数据的地方
	return (uint32_t)pointer_diff(block, block_first) / page->block_size; // block 在第几个块
}

static inline uint32_t
page_block_from_thread_free_list(page_t* page, uint64_t token, block_t** block) {
	uint32_t block_index = (uint32_t)(token & 0xFFFFFFFFULL); // 获取低32位
	uint32_t list_count = (uint32_t)((token >> 32ULL) & 0xFFFFFFFFULL); // 获取token的高32位
...
page_get_local_free_block(page_t* page) {
	block_t* block = page->local_free; // 获取链表 首个空闲
	page->local_free = block->next; // page 指向下一个
	--page->local_free_count; // 空闲 -1
	++page->block_used; // 使用 +1
	return block;
}
...
static inline int	// 如果是被decommit了，那就拿上来
page_commit_memory_pages(page_t* page) {
	if (!page->is_decommitted) // 如果是 0 return 0
		return 0;
	void* extra_page = pointer_offset(page, global_config.page_size); // 忽略
	size_t extra_page_size = page_get_size(page) - global_config.page_size; // 忽略
	if (global_memory_interface->memory_commit(extra_page, extra_page_size) != 0) // 忽略
		return 1;
	...
	void* first_page = pointer_offset(page, PAGE_HEADER_SIZE);	// 从page + 128 开始
...
static void	// 空了 走这函数
page_available_to_free(page_t* page) { // 未满 未被decommit
	...
	heap_t* heap = page->heap; // 获取 heap
	if (heap->page_available[page->size_class] == page) { // 如果是heap正在使用当前 page
		heap->page_available[page->size_class] = page->next; // 将此节点摘除
	} else { // 如果不是正在使用此 page，将此节点摘除
		page->prev->next = page->next; // prev 必不为空
		if (page->next)
			page->next->prev = page->prev;
	}
	page->is_free = 1; // 全空
	page->is_zero = 0; // 非全0 还是老的数据
	page->next = heap->page_free[page->page_type]; // 头插 heap->page_free 对应大小的链表
	heap->page_free[page->page_type] = page; // 头插
...
static void
page_full_to_available(page_t* page) { // 必须是满的，且没 decommit
	rpmalloc_assert(page->is_full == 1, "Page full flag internal failure");
	rpmalloc_assert(page->is_decommitted == 0, "Page decommitted flag internal failure");
	heap_t* heap = page->heap; // 获得heap
	page->next = heap->page_available[page->size_class]; // 头插进入有效队列
	if (page->next) // 如果page->next 存在，也就是heap刚刚在用的
		page->next->prev = page; // 把他.prev 设置为 page
	heap->page_available[page->size_class] = page; // 然后插入
	page->is_full = 0; // 不满了，释放了一个
	if (page->has_aligned_block == 0) // 有没有对齐块
		page->generic_free = 0; // 没有对齐块且不满 则generic_free = 0(large 不走这里)
}
...
page_put_local_free_block(page_t* page, block_t* block) {
	block->next = page->local_free; // 头插 插到page->local_free 里面
	page->local_free = block;	// 头插
	++page->local_free_count;	// free_count++
	if (UNEXPECTED(--page->block_used == 0)) {	// 空了
		page_available_to_free(page);
	} else if (UNEXPECTED(page->is_full != 0)) {	// 满的
...
page_put_thread_free_block(page_t* page, block_t* block) {
	...
	if (page->is_full) { // 如果是满的 加入heap的队列
...
page_allocate_block(page_t* page, unsigned int zero) { // 这个函数是page有空闲才能进入的
	unsigned int is_zero = 0; // 除了前边放地址的地方，后面都是0
	// 这里是如果已经分割了 快递箱情况默认少，local_free 说明是自产自销的 free放入的节点
	block_t* block = (page->local_free != 0) ? page_get_local_free_block(page) : 0;
	if (UNEXPECTED(block == 0)) { // CPU预测是一般分割了的，很少未分割
		if (atomic_load_explicit(&page->thread_free, memory_order_acquire) != 0) { // 有快递
			page_adopt_thread_free_block_list(page); // 
			block = (page->local_free != 0) ? page_get_local_free_block(page) : 0;
		}
		if (block == 0) { // 未切割，未初始化 切block
			block = page_initialize_blocks(page); // 切割，这个函数只切割一部分
			is_zero = page->is_zero; // 
		}
	}
	// 用的块 小于总量
	rpmalloc_assert(page->block_used <= page->block_count, "Page block use counter out of sync");
	if (page->local_free && !page->heap->local_free[page->size_class]) // 如果有free 且 heap的free没有
		page_push_local_free_to_heap(page);	// 把page的free 加入到 heap 里去？
	...
	if (page->block_used == page->block_count) { // 如果都被用了
		page_adopt_thread_free_block_list(page); // 
	...
	if (zero) { // zero 说明 需要置0
		if (!is_zero) // 如果后边不是0
			memset(block, 0, page->block_size);
		else	// 后边是0 只需要把前面地址地方置0
			*(uintptr_t*)block = 0;
	}
...
static inline page_t*	// page_address_mask 根据几个不同大小而不同
...
span_allocate_page(span_t* span) {
	...
	heap_t* heap = span->heap; // 获取span 的 heap		page = 空闲的地址
	page_t* page = pointer_offset(span, span->page_size * span->page_initialized);

#if ENABLE_DECOMMIT // 都忽略了 砍掉
	// The first page is always committed on initial span map of memory
	if (span->page_initialized) { // 如果span 已经初始化了
		if (global_memory_interface->memory_commit(page, span->page_size) != 0) // 忽略
			return 0;
#if RPMALLOC_HEAP_STATISTICS // 忽略
		heap->stats.committed_size += span->page_size;
#endif
	}
#endif
	++span->page_initialized; // 到下一块去
	...
	if (span->page_initialized == span->page_count) { // 如果用完了这个 span
		// Span fully utilized
...
#define HUGE_CACHE_SLOT_COUNT 0	// ###
#endif
#ifndef HUGE_CACHE_COMMITTED_LIMIT
#define HUGE_CACHE_COMMITTED_LIMIT (128 * 1024 * 1024)	// 上限128MB
#endif
...
huge_cache_purge_aged(uint64_t now) {
	span_t* purge[HUGE_CACHE_SLOT_COUNT]; // 32
...
huge_cache_pop(size_t alloc_size) {
	...
	int empty = 1;
	for (uint32_t islot = 0; islot < HUGE_CACHE_SLOT_COUNT; ++islot) { // 32
		if (global_huge_cache.slot[islot].span) {
			empty = 0;
			break;
		}
...
span_deallocate_block(span_t* span, page_t* page, void* block) {
	if (UNEXPECTED(page->page_type == PAGE_HUGE)) { // 是HUGE走下面
#if RPMALLOC_HEAP_STATISTICS // 忽略
		if (span->heap) {
	...
	if (page->has_aligned_block) { // 是对齐块
...
block_deallocate(block_t* block) {
	span_t* span = (span_t*)((uintptr_t)block & SPAN_MASK);	// 看这个block 是哪个 span 的
	page_t* page = span_get_page_from_block(span, block);	// 看这个span的哪个 page 有此block
	const int is_thread_local = page_is_thread_heap(page);	// 看看是不是当前线程的 是则为1

#if RPMALLOC_HEAP_STATISTICS	// 统计 忽略
	...
	if (EXPECTED(is_thread_local != 0)) { // 一般是当前线程m 当前线程f
		if (EXPECTED(page->generic_free == 0)) { // 快速
			// Page is not huge, not full and has no aligned block - fast path
			block->next = page->local_free; // 插入page->local_free 链表
			page->local_free = block;	// 头插
			++page->local_free_count;	// 空闲 +1
			if (UNEXPECTED(--page->block_used == 0)) // 如果释放后全空了
				page_available_to_free(page);	// 
		} else {	// 
			span_deallocate_block(span, page, block); //
		}
	} else { // 别的线程m 出来的 当前线程f
		span_deallocate_block(span, page, block); // 
	}
}
...
heap_lock_acquire(void) {
	uintptr_t lock = 0;
	uintptr_t this_lock = get_thread_id();	// 将锁改成线程id
	while (!atomic_compare_exchange_strong(&global_heap_lock, &lock, this_lock)) {
		lock = 0;	// 失败重新设置为0，因为失败会设置为 global...
		wait_spin();	// 自旋等待
	}
}
...
heap_lock_release(void) {
	rpmalloc_assert((uintptr_t)atomic_load_explicit(&global_heap_lock, memory_order_relaxed) == get_thread_id(),
	                "Bad heap lock"); // 如果释放锁不被我持有 错误 为什么是弱的 因为错误了就等死吧
	atomic_store_explicit(&global_heap_lock, 0, memory_order_release); // 将锁改为0 加内存屏障
}

static inline heap_t*
heap_initialize(void* block) {
	heap_t* heap = block;
	memset_const(heap, 0, sizeof(heap_t)); // memset 从heap开始 设置heap_t 个0
...
heap_allocate_new(void) {
	if (!global_config.page_size) // page_size == 0 说明没进程级初始化
		rpmalloc_initialize(0);
	size_t heap_size = get_page_aligned_size(sizeof(heap_t)); // 堆的大小对齐页的大小(大页或正常页)
	size_t offset = 0;
	size_t mapped_size = 0;
	// mmap 映射
	block_t* block = global_memory_interface->memory_map(heap_size, 0, &offset, &mapped_size);
#if ENABLE_DECOMMIT	// 支持这对函数 commit decommit
	if (global_memory_interface->memory_commit(block, heap_size) != 0) { // commit Linux: 0 删除
		global_memory_interface->memory_unmap(block, offset, mapped_size); 
		return 0;
	}
#endif
	heap_t* heap = heap_initialize((void*)block); // 获取映射后 对堆初始化
	heap->offset = (uint32_t)offset;	// 初始化是 0 
	heap->mapped_size = mapped_size;	// 初始化是 heap_size
#if ENABLE_STATISTICS	// 忽略
...
static heap_t*
heap_allocate(int first_class) { // 线程初始化 传参 0
	heap_t* heap = 0;
	if (!first_class) {
		heap_lock_acquire(); // 锁
		heap = global_heap_queue; // 全局初始 0
		global_heap_queue = heap ? heap->next : 0; // 初始化 0
		heap_lock_release(); // 解锁
	}
	if (!heap)
		heap = heap_allocate_new();	// 初始化走这里
	if (heap) { // heap 存在
		uintptr_t current_thread_id = get_thread_id(); // 获得线程id
		heap_lock_acquire(); // 锁
		heap->next = global_heap_used; // 头插，双向链表
		heap->prev = 0; // 头插 前面为 NULL
		if (global_heap_used) // 如果链表存在元素
			global_heap_used->prev = heap; // 将之前的头.prev 设置为 curr
		global_heap_used = heap; // 更新head
		heap_lock_release(); // 解锁
		heap->owner_thread = current_thread_id; // 线程私有数据设置 id
	}
	return heap;
}
...
heap_make_free_page_available(heap_t* heap, uint32_t size_class, page_t* page) {
	page->size_class = size_class;	// 哪个桶
	page->block_size = global_size_class[size_class].block_size;	// 单个的大小
	page->block_count = global_size_class[size_class].block_count;	// 多少个
	page->block_used = 0;		// 已经使用了的
	page->block_initialized = 0;	// 已经切好了的
	page->local_free = 0;		// free的空闲链表
	page->local_free_count = 0;	// 空闲链表的节点个数
	page->is_full = 0;		// 未满
	page->is_free = 0;		// 未完全空闲
	page->has_aligned_block = 0;	// 
	page->generic_free = 0;		// 通用释放标记
	page->heap = heap;		// 指向当前 heap
	page_t* head = heap->page_available[size_class]; // 头插，获取老的
	page->next = head;		// 头插
	page->prev = 0;			// 头插
	atomic_store_explicit(&page->thread_free, 0, memory_order_release);// 开启收件箱
	if (head) // 如果存在head head->prev = page 头插
		head->prev = page;
	heap->page_available[size_class] = page; // 加入 global_thread... 中
	if (page->is_decommitted != 0) // 得等于 0 才正常返回
...
heap_get_span(heap_t* heap, page_type_t page_type) {
	...
	if (EXPECTED(heap->span_partial[page_type] != 0)) // 预计私有线程存在span
		return heap->span_partial[page_type]; // 直接获取对应大小的span

	// Fallback path, map more memory
	size_t offset = 0;
	size_t mapped_size = 0;				// 256MB	256MB
	span_t* span = global_memory_interface->memory_map(SPAN_SIZE, SPAN_SIZE, &offset, &mapped_size);
#if RPMALLOC_HEAP_STATISTICS	// 忽略
	heap->stats.mapped_size += mapped_size;
#endif
	if (EXPECTED(span != 0)) { // 预计是可以获得的 虚拟地址不太可能用得完
		uint32_t page_count = 0;
		uint32_t page_size = 0;
		uintptr_t page_address_mask = 0;
		if (page_type == PAGE_SMALL) { // 小 页
			page_count = SPAN_SIZE / SMALL_PAGE_SIZE; // 2^28 / 2^16 = 2^12
		...
		span->page_type = page_type; // span 的所属类型
		span->page_count = page_count; // 有多少个这个类型的块
		span->page_size = page_size; // 所属类型的大小
		span->page_address_mask = page_address_mask; // 掩码
		span->offset = (uint32_t)offset; // offset
		span->mapped_size = mapped_size; // mapped_size
...
heap_get_page_generic(heap_t* heap, uint32_t size_class) {
	page_type_t page_type = get_page_type(size_class); // 获得size_class 对应标签

	// Check if there is a free page from multithreaded deallocations
	// 只要求原子读，其他乱序不要求 看看对应的快递数组有没有东西
	uintptr_t block_mt = atomic_load_explicit(&heap->thread_free[page_type], memory_order_relaxed);
	if (UNEXPECTED(block_mt != 0)) { // 预测一般不存在东西，如果存在
		while (!atomic_compare_exchange_weak_explicit(&heap->thread_free[page_type], &block_mt, 0, memory_order_acquire,
		                                              memory_order_relaxed)) {
			wait_spin(); // CAS 失败了也会把最新的更新到block_mt 里，成功就设置0
		}
		block_t* block = (void*)block_mt;
		while (block) { // 拿到快递后
			block_t* next_block = block->next; // 链表的下一个
			block_deallocate(block); // 重新free
			block = next_block; // 往后走
		}
		// Retry after processing deferred thread frees
		return heap_get_page(heap, size_class); // 复活后重试
	}

	// Check if there is a free page
	page_t* page = heap->page_free[page_type]; // 自己有空闲page
	if (EXPECTED(page != 0)) { // 一般是有的
		heap->page_free[page_type] = page->next; // 获取page page_free指向下一个page
		if (page->is_decommitted == 0) { // 被虚拟内存系统解除了绑定吗 0表示没有
	...
	if (EXPECTED(span != 0)) { // 层级: span -> page -> block
...
static inline RPMALLOC_ALLOCATOR void*
heap_pop_local_free(heap_t* heap, uint32_t size_class) {	// 获取桶内装的固定小空间
	block_t** free_list = heap->local_free + size_class;	// 拿到桶的地址
	block_t* block = *free_list;				// 获得桶的第一个元素
	if (EXPECTED(block != 0))				// 预测桶不为空(CPU流水线预测)
		*free_list = block->next;			// 将空闲指向下一位
	return block;
}
...
heap_allocate_block_small_to_large(heap_t* heap, uint32_t size_class, unsigned int zero) {
	page_t* page = heap_get_page(heap, size_class); // 获取一个页
	if (EXPECTED(page != 0))	// 预计成功
		return page_allocate_block(page, zero); // 分割
	return 0;
}
...
heap_allocate_block(heap_t* heap, size_t size, unsigned int zero) {
	if (size <= (SMALL_GRANULARITY * 64)) { // 如果申请的大小 小于16 个cache大小
		uint32_t size_class = get_size_class_tiny(size);
		block_t* block = heap_pop_local_free(heap, size_class);
		if (EXPECTED(block != 0)) { // 如果在上面这行获得了
			// Fast track with small block available in heap level local free list
			if (zero) // 如果传入的是非0 将申请的块全部置0
				memset(block, 0, global_size_class[size_class].block_size);
#if RPMALLOC_HEAP_STATISTICS
			heap->stats.allocated_size += global_size_class[size_class].block_size;
#endif
...
rpmalloc_initialize(rpmalloc_interface_t* memory_interface) {
	if (global_rpmalloc_initialized) { // 全局初始化只需要一次
		rpmalloc_thread_initialize(); // 现在只需要线程初始化了
		return 0;
	}
	// 开始进程级初始化，在最后 线程级初始化 这可能出现多个线程同时到此
	global_rpmalloc_initialized = 1; // 全局初始化了(进程级)
...
#elif defined(__APPLE__) || defined(__NetBSD__)	// 忽略
		os_huge_pages = 1;
		os_page_size = 2 * 1024 * 1024;
		os_map_granularity = os_page_size;
#endif
...
#if defined(__linux__) || defined(__ANDROID__)	// Linux 安卓
	...
	rpmalloc_thread_initialize(); // 最后将首次全局初始化的线程给初始化
